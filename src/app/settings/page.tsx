...
import { 
  ...
  Bold, Heading1, Heading2, List, Palette, Eye, Edit3, Type, Eraser, Highlighter, ListOrdered, History, TrendingUp, Search, Printer, Calendar as CalendarIcon, XCircle
} from "lucide-react"
...
export default function SettingsPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8 pb-20">
      ...
      {/* Official Print Header for Food History (Only visible in print) */}
      <div className="print-only print-report-container">
        <div className="report-header text-center">
          <h1 className="text-2xl font-black uppercase text-primary">SOMIKORON HOSTEL</h1>
          <p className="text-sm font-bold">{userBranch} Branch • Daily Food Cost Analysis</p>
          <div className="mt-4 border-y py-2 grid grid-cols-2 text-left text-[10pt]">
            <div><p><b>Report Period:</b> {foodStartDate} to {foodEndDate}</p></div>
            <div className="text-right"><p><b>Generated By:</b> {userName}</p><p><b>Date:</b> {new Date().toLocaleDateString()}</p></div>
          </div>
        </div>
        <Table className="border w-full">
          <TableHeader>
            <TableRow className="bg-slate-50">
              ...
              <TableHead className="border">Total Meals</TableHead>
              <TableHead className="border">Total Cost (৳)</TableHead>
              <TableHead className="border text-right">Per Meal Price</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {filteredFoodHistory.map((item) => {
              ...
              return (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="border text-center">{totalMeals}</TableCell>
        ...
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Dialog open={isFoodHistoryOpen} onOpenChange={setIsFoodHistoryOpen}>
            ...
            <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              {/* Filter Bar */}
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-secondary/20 p-4 rounded-xl border items-end my-4">
                <div className="space-y-1.5">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground ml-1">From Date</Label>
                  <Input type="date" value={foodStartDate} onChange={e => setFoodStartDate(e.target.value)} className="bg-white h-9" />
                </div>
                <div className="space-y-1.5">
                  <Label className="text-[10px] uppercase font-bold text-muted-foreground ml-1">To Date</Label>
                  <Input type="date" value={foodEndDate} onChange={e => setFoodEndDate(e.target.value)} className="bg-white h-9" />
                </div>
              ...
              <DialogFooter>
                <Button onClick={() => setIsFoodHistoryOpen(false)} variant="secondary" className="w-full">Close Report</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </CardHeader>
