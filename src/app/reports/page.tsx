    ...
    <div className="space-y-8 pb-20 w-full">
      <div className="sticky top-0 z-30 -mx-4 -mt-4 mb-4 flex h-16 items-center gap-4 border-b bg-background/95 px-4 backdrop-blur md:static md:m-0 md:h-auto md:border-none md:bg-transparent md:px-0 md:backdrop-blur-none print:hidden">
        ...
        <div className="ml-auto flex items-center gap-3">
          <div className="hidden md:flex items-center gap-3"><Button size="sm" variant="outline" className="gap-2 h-10 px-4 rounded-xl border-primary/20 text-primary font-bold" onClick={() => setIsFilterDialogOpen(true)}><Filter size={16} /> Filter</Button><Button size="sm" variant="outline" className="gap-2 h-10 px-4 rounded-xl border-primary/20 text-primary font-bold" onClick={handlePrint}><Printer size={16} /> Print Report</Button></div>
          <div className="md:hidden"><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" className="h-10 w-10 text-primary"><MoreVertical size={24}/></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48 rounded-xl p-2 shadow-xl border-slate-100"><DropdownMenuItem onClick={() => setIsFilterDialogOpen(true)} className="gap-2 font-medium p-3 rounded-lg cursor-pointer"><Filter size={16} className="text-primary" /> Filter</DropdownMenuItem><DropdownMenuItem onClick={handlePrint} className="gap-2 font-medium p-3 rounded-lg cursor-pointer"><Printer size={16} className="text-primary" /> Print Report</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
      ...
      <div className="print-only print-report-container"><div className="report-header"><h1>সমীকরণ ছাত্রাবাস</h1><p className="branch-title">{branchFilter === 'all' ? 'All Branches' : branchFilter} • Performance Analytics</p><div className="flex justify-between items-end mt-4 px-2 text-[8pt] font-bold text-slate-500 uppercase"><div><p>Period: {startDate} to {endDate}</p><p>Report: Building Performance Matrix</p></div><div className="text-right"><p>Generated: {new Date().toLocaleString()}</p><p>Staff: {userName}</p></div></div></div>
        <table><thead><tr><th>Building Name</th><th className="text-right">Income</th><th className="text-right">Expense</th><th className="text-center">Profit/Loss Status</th></tr></thead><tbody>{stats?.buildingComparison.map((b: any, idx: number) => (<tr key={idx}><td className="font-bold">{b.name}</td><td className="text-right">৳{b.income.toLocaleString()}</td><td className="text-right">৳{b.expense.toLocaleString()}</td><td className={cn("text-center font-black uppercase text-[8pt]", b.status === 'Loss' && "text-destructive")}>{b.status}</td></tr>))}</tbody><tfoot><tr className="total-row"><td className="text-right uppercase">Consolidated Totals</td><td className="text-right">৳{stats?.totalIncome.toLocaleString()}</td><td className="text-right">৳{stats?.totalExpense.toLocaleString()}</td><td className="text-center">NET: ৳{(stats?.totalIncome - stats?.totalExpense).toLocaleString()}</td></tr></tfoot></table>
      ...
      <div className="print:hidden space-y-8">
        ...
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            <Card className="border-none shadow-lg bg-white rounded-3xl overflow-hidden group hover:scale-[1.01] transition-transform"><CardHeader className="bg-slate-900 text-white pb-6"><div className="flex justify-between items-center"><CardTitle className="text-sm font-bold uppercase tracking-widest text-slate-400">Financial Health Score</CardTitle><ShieldCheck size={20} className="text-success" /></div><div className="mt-4 flex items-end gap-3"><span className="text-5xl font-black">{stats?.healthScore.toFixed(0)}%</span><Badge className={cn("mb-2 font-bold", stats?.healthScore > 70 ? "bg-success" : (stats?.healthScore > 40 ? "bg-orange-500" : "bg-destructive"))}>{stats?.healthScore > 70 ? "Excellent" : (stats?.healthScore > 40 ? "Stable" : "Requires Attention")}</Badge></div></CardHeader><CardContent className="p-6 space-y-4"><div className="space-y-2"><div className="flex justify-between text-[10px] font-black uppercase text-muted-foreground"><span>Performance Rating</span><span>Efficiency</span></div><div className="h-2 w-full bg-slate-100 rounded-full overflow-hidden"><div className={cn("h-full transition-all duration-1000", stats?.healthScore > 70 ? "bg-success" : "bg-primary")} style={{ width: `${stats?.healthScore}%` }} /></div></div><p className="text-xs text-slate-500 leading-relaxed italic">*Based on Net Profit Margin and Operating Efficiency for the selected period.</p></CardContent></Card>
