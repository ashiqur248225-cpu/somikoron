      ...
      <Card className="border-none shadow-xl rounded-3xl overflow-hidden bg-white">
        ...
        <CardHeader>
          <CardTitle className="text-xl font-bold flex items-center gap-2 text-destructive">
            <Receipt size={24}/> New Expense Record
          </CardTitle>
        </CardHeader>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-4 p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  <div className="space-y-1.5">
                    ...
                    <Input type="date" value={formData.expenseDate} onChange={e => setFormData({...formData, expenseDate: e.target.value})} className="bg-white h-12 rounded-xl shadow-sm" />
                  </div>
                  <div className="space-y-1.5">
                    <Label className="text-xs">Category</Label>
                    <Select value={formData.category} onValueChange={v => setFormData({...formData, category: v, buildingId: "none", apartmentName: "", roomNumber: "", receiver: "", totalMeals: ""})}>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input type="number" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} className="bg-white h-14 rounded-xl text-2xl font-black text-destructive shadow-sm" placeholder="0.00" />
                  </div>
                </div>
              </div>
                ...
                <div className="grid grid-cols-1 gap-4 p-6 bg-slate-50 rounded-3xl border border-slate-100">
                  <div className="space-y-1.5">
                    ...
                    <Select value={formData.method} onValueChange={v => setFormData({...formData, method: v})}>
                      <SelectTrigger className="bg-white h-12 rounded-xl shadow-sm"><SelectValue /></SelectTrigger>
              ...
              <div className="space-y-4">
                ...
                <div className="p-6 bg-primary/5 rounded-3xl border border-primary/10 space-y-5 min-h-[400px] shadow-inner">
                  {['rent', 'electricity', 'water', 'maintenance', 'others'].includes(formData.category) && (
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <Select value={formData.buildingId} onValueChange={v => setFormData({...formData, buildingId: v, apartmentName: "", roomNumber: ""})}>
                          <SelectTrigger className="bg-white h-11 rounded-xl shadow-sm"><SelectValue placeholder="Building" /></SelectTrigger>
                          <SelectContent>
                      ...
                      {formData.category === 'electricity' && (
                        <div className="space-y-1.5"><Label className="text-xs">Meter Number</Label><Input value={formData.meterNo} onChange={e => setFormData({...formData, meterNo: e.target.value})} className="bg-white h-11 rounded-xl shadow-sm" /></div>
                      )}
                      {['maintenance', 'others'].includes(formData.category) && (
                        <div className="space-y-1.5"><Label className="text-xs">Room / Unit</Label><Input value={formData.roomNumber} onChange={e => setFormData({...formData, roomNumber: e.target.value})} className="bg-white h-11 rounded-xl shadow-sm" /></div>
                      )}
                    </div>
                  )}
                  ...
                  {formData.category === 'salary' && (
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <Select value={formData.month} onValueChange={v => setFormData({...formData, month: v})}><SelectTrigger className="bg-white h-11 rounded-xl shadow-sm"><SelectValue /></SelectTrigger><SelectContent>{MONTHS.map(m => <SelectItem key={m} value={m}>{m}</SelectItem>)}</SelectContent></Select>
                        <Select value={formData.year} onValueChange={v => setFormData({...formData, year: v})}><SelectTrigger className="bg-white h-11 rounded-xl shadow-sm"><SelectValue /></SelectTrigger><SelectContent>{YEARS.map(y => <SelectItem key={y} value={y}>{y}</SelectItem>)}</SelectContent></Select>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="space-y-1.5 pt-4">
                    ...
                    <Textarea value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Details..." className="bg-white rounded-2xl resize-none min-h-[120px] shadow-sm" />
                  </div>
                </div>
              </div>
            </div>
        ...
        <CardFooter className="p-10 bg-slate-50 border-t">
          <Button onClick={handleCreateExpense} disabled={isSubmitting} className="w-full h-20 rounded-3xl text-2xl font-black bg-destructive hover:bg-destructive/90 shadow-2xl shadow-destructive/20 gap-3">
            {isSubmitting ? <Loader2 className="animate-spin" /> : <Receipt size={32}/>} Record Final Expense
          </Button>
        </CardFooter>
      </Card>
    </div>
